...
def test_apiclientmethods():
    ...
    exp = "s_sessionid=12345678&s_command=COMMAND%3DStatusAccount"
    assert tmp == exp
    ...
    cl.enableDebugMode()
    tmp = cl.getPOSTData({"COMMAND": "StatusAccount"})
    exp = "s_login=myaccountid%3Amyrole&s_sessionid=12345678&s_command=COMMAND%3DStatusAccount"
    assert tmp == exp
    ...
    # [session and login set should consider the login]
    cl.setSession("12345678")
    cl.setCredentials("myaccountid", "mypassword")
    tmp = cl.getPOSTData({"COMMAND": "StatusAccount"})
    assert tmp == "s_login=myaccountid&s_pw=mypassword&s_command=COMMAND%3DStatusAccount"

    # #.saveSession/reuseSession
    sessionobj = {}
    cl.setCredentials(
        "myaccountid").setSession("12345678").saveSession(sessionobj)
    cl2 = AC()
    ...
    sessionobj = {}
    cl.setSession("12345678").setCredentials("myaccountid", "mypassword")
    tmp = cl.getPOSTData({"COMMAND": "StatusAccount"})
    ...
    cl.enableDebugMode()
    cl.setSession(tpl.getRecord(0).getDataByKey("SESSIONID"))
    r = cl.logout()
    assert isinstance(r, R) is True
    assert r.isError() is True
